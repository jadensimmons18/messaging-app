...
- [ ] **Phase 3 (REST messaging) — in progress.** `getOrCreateConversation` controller logic is written and looks correct, but **not yet tested end-to-end** — see build notes below for a currently-blocking bug in `conversationRoutes.js`.
- [ ] `fetch message history`, `send a message`, `listConversations` — not started yet, next after `getOrCreateConversation` is actually verified working
- [ ] Everything from Phase 4 onward — not started
...
**⚠️ Currently blocking, not yet fixed:** `conversationRoutes.js` still has the exact same class of bugs already caught once in `contactRoutes.js` back in Phase 2 — `import express from 'mongoose'` (wrong package as the import source) and an incomplete `router.post('')` with no handler function, no `authMiddleware`/`getOrCreateConversation` imports, and no `export default`. This is now actively imported by `server.js` (already wired in: `app.use('/api/conversation', conversationRoutes)`), meaning **the server currently fails to boot at all** until this file is fixed. This is the very next thing to fix before anything in Phase 3 can be tested.
