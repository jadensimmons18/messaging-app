...
- [ ] **Phase 4 (Socket.io) — in progress.** Scope-restructuring decision made mid-phase — see "⚠️ Scope Change" above. `server.js` restructured: `createServer(app)`/`new Server(httpServer, {cors})` at module scope (the earlier block-scoping bug noted here was already fixed by the time this was picked back up — confirmed by reading the actual file rather than trusting this checklist, same "verify, don't trust the doc" habit as everywhere else). **Handshake authentication — done and verified.** **Rooms (`join_conversation`) — done and verified.** Socket logic split into its own `backend/sockets/socketHandlers.js` (see build notes below). Only the `send_message`/`receive_message` emit pattern remains. **This is the next work to pick up.**
...
### Phase 3 (REST Messaging) build notes, so far

**Major design decision — no gate on who can message whom, unlike the original assumption.** Earlier (Phase 2 notes) it was assumed `Contact` existing as `accepted` should gate conversation creation. Jaden overturned this deliberately: **anyone can message anyone**, no restriction — messaging behaves like Instagram DMs, not a closed contacts-only system. The `Contact` accept/reject flow instead becomes the mechanism for a frontend "Unknown Messages" vs. "Contacts" tab split:
- `getOrCreateConversation` auto-creates a `pending` `Contact` between the two people **only if no `Contact` already exists at all** (neither `pending` nor `accepted`) — it never blocks conversation creation, regardless of `Contact` status.
- The frontend's "Accept"/"Reject" buttons on an unknown message **reuse the existing `acceptFriend`/`rejectFriend` endpoints directly** — no new endpoints needed. Accepting a message *is* accepting the underlying pending `Contact`; rejecting deletes it. This was almost built wrong — Jaden's first instinct was to have "accept message" call `addFriend`, which would actually fail: `addFriend`'s duplicate-check would see the already-existing pending `Contact` and reject with `409`, since `addFriend` is for *initiating* a request, not accepting one that already exists.
- **Which tab a conversation belongs in is computed dynamically at `listConversations` time** (not built yet) by checking the live `Contact.status` between the two participants — deliberately **not** cached as a field on `Conversation` itself. Jaden proposed caching it; rejected for the same dual-write-consistency reason `User.contacts` arrays were rejected — a cached copy on `Conversation` would silently go stale the moment `acceptFriend`/`rejectFriend` updates `Contact` without also remembering to update every `Conversation` between those two people. `Contact.status` stays the single source of truth; the tab is a read-time lookup, not stored state.
- No self-messaging restriction (unlike `addFriend`'s self-request `400`) — Jaden correctly pointed out real apps (iMessage Notes to Self, WhatsApp/Telegram Message Yourself) support this, and there's no reason to block it here.

**`getOrCreateConversation` query design:** looked for an existing 1:1 conversation using an **exact-array-match `$or`** (`{ participants: [req.userId, participantId] }` OR `{ participants: [participantId, req.userId] }`), not `$all`/`$size` as first proposed. Reasoning worth remembering: `$all` treats a value listed twice as identical to listing it once (it doesn't require the duplicate to actually appear twice in the array) — so for the self-messaging case specifically, `{ $all: [userId, userId], $size: 2 }` would incorrectly match *any* 2-person conversation containing that user, not specifically a genuine self-conversation. The exact-match `$or` version sidesteps this entirely: when messaging yourself, both `$or` branches collapse to the identical exact match `[userId, userId]`, which can only ever match a real self-conversation.

**Real bugs hit while building this — same "copy-pasted from `addFriend` without adapting" pattern showed up repeatedly:**
1. Tried calling `addFriend()` directly (with no arguments, then later with `addFriend(req, res)`) to reuse its contact-creation logic — doesn't work, and it's worth remembering exactly why: route handlers are the *sole* responder for one HTTP request. `addFriend` reads `req.body.recipient` (this endpoint's body has `participantId`, a field-name mismatch causing `undefined`), and more fundamentally, it calls `res.json()` itself — if `getOrCreateConversation` also tried to send its own response afterward, that's the same `ERR_HTTP_HEADERS_SENT` double-response crash from Phase 1, since two different pieces of code would both try to respond to the same request. The actual fix: inline just the small amount of logic actually needed (check-then-conditionally-create), not the whole `addFriend` handler.
2. Multiple undeclared-variable `ReferenceError`s from copy-pasting `addFriend`'s logic without renaming: `recipient` used instead of `participantId`, `userId` used instead of `req.userId`, `conversation` referenced instead of the actually-declared `existingConversation`, and `Contact` used without ever being imported into the file.
3. **An early `return` after creating the pending contact made the actual `Conversation.create()` call unreachable** — copied `addFriend`'s own `201` success response into this function, which exited before ever reaching the conversation-creation code below it. Fix: the contact-creation step needed to be a silent side effect with no response of its own, not a second competing "success" response.
4. **A `409`-and-return on finding an existing `Contact` blocked conversation creation entirely** — copied from `addFriend`, where an existing contact genuinely means "nothing left to do." Here it's wrong: an existing contact (even `'accepted'`) should just mean "skip creating a *new* one," not "refuse to create the conversation." Wrapped the contact-creation in `if (!existingContact) { ... }` with no early return, so conversation creation always proceeds regardless.
5. **Block-scoping bug**: `const newContact` declared inside that `if (!existingContact) { ... }` block was then referenced in the final response *outside* the block — `const`/`let` are block-scoped, so this threw `ReferenceError` on every request once the `409` bug above was fixed (since execution now always reached that line). Resolved by dropping `newContact` from the response entirely — this endpoint's job is conversations, not contacts, so it doesn't need to report on the contact either way.

**`conversationRoutes.js` blocking bug — now fixed and verified.** It had the exact same class of bugs already caught once in `contactRoutes.js` back in Phase 2 (`import express from 'mongoose'`, incomplete `router.post('')` with no handler). Fixed; `POST /api/conversation` (create) and `GET /api/conversation` (list) both mounted correctly.

**`loadMessageHistory` and `sendMessage`** (`backend/controllers/messageController.js`, `backend/routes/messageRoutes.js`, mounted as `app.use('/api/message', messageRoutes)`): both share the same **conversation-membership authorization check** — fetch the `Conversation` by `req.params.conversationId`, `404` if missing, then `conversation.participants.some(p => p.toString() === req.userId)`, `403` if the requester isn't actually a participant. Noted as a duplication candidate for a shared helper once a third place needs it, but left inline for now since it's only used twice so far.

- `loadMessageHistory`: `GET /api/message/:conversationId`. Paginates **newest-first** (`sort({ createdAt: -1 })`), *opposite* of the ascending-by-recency pagination used elsewhere — chat histories load the most recent messages first, then page backward into the past as the user scrolls up. Populates `sender` (`username avatarUrl`) so the frontend isn't stuck with bare IDs. Verified with 25 seeded messages: page 1 returns the newest 20 (`#25` down to `#6`), page 2 returns the remaining 5, an outsider (non-participant) gets `403`, a bogus conversation ID gets `404`.
- `sendMessage`: `POST /api/message/:conversationId`. Creates the `Message`, then **updates `Conversation.lastMessage`** — the first real payoff of that field, which was added speculatively all the way back when `Conversation` was first designed. `catch` differentiates `ValidationError` (empty/missing `content`, or over the 5000-char `maxlength`) → `400` from anything else → `500`. Verified: two real messages send correctly (`201`), an outsider is blocked (`403`), empty content is rejected (`400`), and `Conversation.lastMessage` was confirmed in the database to actually point at the most recent message afterward.

**`listConversations`** (`GET /api/conversation`) — the most involved piece this phase, built in two passes:
1. **The base query** uses a new pattern: `Conversation.find({ participants: req.userId })` — comparing an array field to a single, non-array value automatically means "does this array contain this value," no `$in`/`$all` needed. Populates `participants` and `lastMessage`, sorts by `updatedAt: -1` (works because `{ timestamps: true }` bumps `updatedAt` on every `.save()`, and `sendMessage` already saves the conversation when it updates `lastMessage` — so sending a message naturally bumps that conversation to the top with zero extra code).
2. **Per-conversation transformation via `.map()`** (first real use of it — different from `.find()`/`.some()` since it returns a new array of the *same length*, one transformed entry per original): for each conversation, find "the other participant" (`.find(p => p._id.toString() !== req.userId)`), falling back to `c.participants[0]` for the self-messaging case where no "other" exists.
3. **Contact-vs-Unknown tab classification, computed live, not cached** (per the earlier design decision): fetch all `accepted` `Contact`s involving the user, map each to "the other person's ID," and collect those into a **`Set`** — chosen specifically because the function needs to repeatedly ask "is this person one of my accepted contacts?" once per conversation, and a `Set`'s `.has()` is effectively instant regardless of size, unlike scanning a plain array with `.includes()` on every check. Same underlying idea as a database index, just applied to an in-memory JS collection instead of MongoDB.
4. **Self-conversations are always treated as a contact** (`isSelfConversation = otherParticipant._id.toString() === req.userId`, then `isContact = isSelfConversation || contactIds.has(...)`) — a deliberate fix after testing showed "Notes to Self" would otherwise land in the "Unknown Messages" tab, which makes no sense for messaging yourself.

Verified end-to-end with a 3-conversation scenario (one accepted contact, one unknown/pending sender, one self-conversation): correct recency ordering, correct `isContact` flag on all three (including the self-conversation fix), correct `otherParticipant` identification on each.

**Real bugs hit while building this phase, beyond the ones already listed above:**
1. **Arrow function with a block body but no `return`** — `.find((p) => { p._id.toString() !== req.userId })` (curly braces with no `return` statement) always returns `undefined`, since switching to a block body turns off the implicit-return behavior. `.find()` then never matches anything, regardless of the actual data. Fixed by dropping the braces for an implicit return.
2. **A fallback (`|| c.participants[0]`) accidentally placed *inside* the `.find()` callback instead of *after* the whole call** — because `!==` binds tighter than `||`, `p._id.toString() !== req.userId || c.participants[0]` was evaluated per-element, meaning it returned a truthy value (the fallback participant) even for the *first* element checked, incorrectly matching yourself as "the other participant" in every conversation where you happen to be listed first in `participants` (which is most of them, since `getOrCreateConversation` always creates `[req.userId, participantId]`). Fixed by moving the fallback outside the `.find()` call entirely.
3. **Route path missing its route parameter** (`router.get('/loadMessageHistory', ...)` instead of `'/:conversationId'`) — a static path has no "slot" for the ID to occupy, so a real request like `/api/message/64f8a3...` wouldn't match the route at all (`404`), not just receive `undefined` for the param.
4. **Route path missing its leading slash** (`router.get(':conversationId', ...)`) — Express route paths need to start with `/`.
5. **Default-vs-named import mismatch**: `messageController.js` exports `loadMessageHistory` as a named export, but an early draft of `messageRoutes.js` imported it with default-import syntax (`import loadMessageHistory from ...`) — doesn't work, since there's no `export default` in that file to match against.
6. `conversationId` typo'd as `conversationID` (wrong casing) inside `sendMessage`, and `Message.create(...)` missing `await` in the same line — same "copy-paste without adapting" and "forgot await on a Promise-returning call" patterns from earlier phases, still worth double-checking on every new handler.

### Phase 4 (Socket.io) build notes, so far

**Structure decision — socket logic split out of `server.js`, mirroring route/controller separation.** `backend/sockets/socketHandlers.js` exports a single function, `registerSocketHandlers(io)`, containing everything socket-related (handshake middleware, `io.on('connection', ...)`, and every `socket.on(...)` listener). `server.js` just creates `io` and calls `registerSocketHandlers(io)` once — same "entry point stays thin, real logic lives elsewhere" principle already used for `routes/*.js`.

**Handshake authentication — done and verified.** `io.use((socket, next) => {...})` reads `socket.handshake.auth.token` (where the frontend will need to put its JWT when connecting), verifies it with the same `jwt.verify()` call `authMiddleware` uses, and sets `socket.userId` on success — the direct socket-world equivalent of `req.userId`. Missing or invalid token → `next(new Error(...))`, which rejects the connection at handshake time, before `'connection'` ever fires.

**Rooms (`join_conversation`) — done and verified.** `socket.on('join_conversation', async (conversationId) => {...})`, registered inside `io.on('connection', ...)`. Fetches the `Conversation` by id, and reuses the exact same authorization pattern as `loadMessageHistory`/`sendMessage` (`conversation.participants.some(p => p.toString() === socket.userId)`) before allowing `socket.join(conversationId)` — a socket being authenticated says nothing about whether it's allowed into *this* room, same authentication-vs-authorization split as REST. **New pattern, socket-specific: there's no `res`.** A socket handler can't send a status code; feedback goes back to that one client by emitting a distinct event instead — `socket.emit('joined_conversation', conversationId)` on success, `socket.emit('error', { message })` for "not found" or "not authorized."

**Real bug hit — not a code bug, an environment one.** `backend/.env` had ended up sitting at the project root instead of inside `backend/`. `dotenv` resolves `.env` relative to the process's **current working directory at runtime**, not relative to wherever `server.js` itself lives — so running `node server.js` from inside `backend/` silently found no `.env` at all, and every `process.env.*` read (including `MONGO_URI`) came back `undefined`. This didn't surface as an obvious "env missing" error — it came out as `MongooseError: The uri parameter to openUri() must be a string, got "undefined"`, which reads like a code bug at first glance. Fixed by moving the file to `backend/.env` (confirmed it's still correctly gitignored there — `.gitignore`'s unanchored `.env` line matches at any depth). Also had to run `npm install` before any of this — `node_modules` wasn't present in the working tree at all.

**Verified end-to-end, same discipline as every prior phase.** Booted the real server, signed up three throwaway users via live `curl` calls, created a real `Conversation` between two of them via `POST /api/conversation`, then drove five real socket connections (via a temporary `socket.io-client` dev dependency, removed afterward) against the running server: no token → rejected at handshake; garbage token → rejected at handshake; the real participant joining their own conversation → `joined_conversation`; an unrelated third user trying to join that same conversation → `error` (blocked correctly); joining a nonexistent conversation id → `error` ("not found"). All five passed. Test users, the auto-created pending `Contact` between them (confirms `getOrCreateConversation`'s side effect fired), the test `Conversation`, and the temporary dependency were all cleaned up afterward — nothing left behind in the real database or `package.json`.
...
- Comparing an array field directly to a single (non-array) value — `Conversation.find({ participants: req.userId })` — automatically means "does this array contain this value." No `$in`/`$all` needed for a single-value containment check; those are for matching against multiple possible values.
- Chat-style message history paginates **newest-first** (`sort({ createdAt: -1 })`) — the opposite direction from list-style pagination (search results, contact requests), since users scroll *backward in time* from "most recent," not forward from "page 1."
- `.map()` is for transforming every element of an array into a new array of the same length (e.g. reshaping raw conversation documents into a cleaner response shape) — distinct from `.find()` (one matching element or `undefined`) and `.some()` (a boolean). Pick based on what the result should look like: one item, yes/no, or a full transformed list.
- Reach for a `Set` when a piece of code needs to repeatedly ask "is this value in this collection?" — `.has()` is effectively instant regardless of size, unlike scanning a plain array with `.includes()` on every check. Same underlying idea as a database index, applied to an in-memory JS collection instead.
- Any handler that reads or writes something inside an existing `Conversation` (message history, sending a message) must verify the requester is actually one of its `participants` — same authentication-vs-authorization distinction as `Contact`, just checked via `.some()` over an array instead of a single field.
- Socket event handlers live in `backend/sockets/socketHandlers.js`, structured as a single exported `registerSocketHandlers(io)` function called once from `server.js` — mirrors route/controller separation: `server.js` stays a thin entry point, real socket logic lives elsewhere.
- Socket handlers reuse the same authentication-vs-authorization split as REST: handshake middleware (`io.use(...)`) verifies the JWT once per connection and sets `socket.userId`, exactly like `authMiddleware` sets `req.userId` — but a handler acting on a specific resource (e.g. `join_conversation`) still needs its own authorization check (participant membership via `.some()`), same as any REST handler touching an existing document. There's no `res` in a socket handler — feedback goes back to the client via emitting a distinct event (e.g. `joined_conversation` vs `error`) instead of an HTTP status code.
...
- Don't reuse another route handler by calling it directly (e.g. `addFriend(req, res)`) to share logic — a handler is the sole responder for one request, and its `req.body` shape belongs to *its own* route, not whatever's calling it. Two handlers both trying to send a response for the same request crashes with `ERR_HTTP_HEADERS_SENT`. Extract shared logic into a plain function instead, or just inline the small amount actually needed.
- Don't assume MongoDB's `$all` requires a duplicated value to appear twice in the target array — `{ $all: [x, x] }` behaves identically to `{ $all: [x] }`; duplicates in the query are ignored. Combined with `$size`, this can silently match unintended documents (caught this with a self-messaging conversation lookup that would've matched *any* 2-person conversation containing that user). An exact-array match (`{ field: [x, y] }`) is the right tool when both order-independence and duplicate-safety matter.
- Don't give an arrow function a block body (`{ }`) without an explicit `return` — this silently disables the implicit-return behavior, and the function just returns `undefined` no matter what. `.find((p) => { p._id.toString() !== req.userId })` (no `return`) always comes back empty, regardless of the actual data — nothing throws, it just quietly never matches.
- Don't place a `||` fallback *inside* a `.find()`/`.some()` callback expecting it to apply to the overall search result — operator precedence means it gets evaluated per-element instead. `p._id.toString() !== req.userId || fallback` runs the fallback check on *every single element*, not just when nothing matches; the fallback needs to sit outside the closing `)` of `.find(...)` to apply to the whole call's result.
- Don't forget a route path needs an actual `:paramName` slot for `req.params` to have anything in it — a static path like `/loadMessageHistory` won't just yield `undefined` for a missing param, real requests carrying an ID in the URL won't match the route at all (`404`).
- Don't mix up default and named exports/imports — a file using `export const x = ...` requires `import { x } from ...` on the other end; `import x from ...` (default syntax) will not find it.
- Don't assume `dotenv` finds `.env` based on where the script file lives — it resolves `.env` relative to the process's **current working directory at runtime**, not the script's own location. `backend/.env` ending up at the project root instead of inside `backend/` meant running `node server.js` from inside `backend/` silently got every `process.env.*` value as `undefined`, which surfaced as a confusing `MongooseError: ... got "undefined"` rather than any obviously env-related error.
